...
create policy "Service role can create exports"
  ...
  for insert
  to service_role
with check ((bucket_id = 'resume-exports'::text));
create policy "Users can delete their own exports"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
using (((bucket_id = 'resume-exports'::text) AND ((storage.foldername(name))[1] = (auth.uid())::text)));
create policy "Users can delete their own resumes"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
using (((bucket_id = 'resume-uploads'::text) AND ((storage.foldername(name))[1] = (auth.uid())::text)));
...
create policy "Users can update their own resumes"
  ...
  to authenticated
using (((bucket_id = 'resume-uploads'::text) AND ((storage.foldername(name))[1] = (auth.uid())::text)))
with check (((bucket_id = 'resume-uploads'::text) AND ((storage.foldername(name))[1] = (auth.uid())::text)));
