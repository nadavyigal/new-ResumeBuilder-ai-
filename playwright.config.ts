...
export default defineConfig({
  ...
  // No retries. Every network call in these specs is intercepted and the suite
  // runs in about seven seconds, so there is no infrastructure nondeterminism for
  // a retry to absorb -- it could only hide a real intermittent bug in the funnel
  // by passing on the second attempt. A gate you clear by trying again is not one.
  retries: 0,
